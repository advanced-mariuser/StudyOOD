...
class Toolbar extends Component<ToolbarProps> {
	...
	override render() {
		return (
			<div style={{display: 'flex', gap: '10px', fontFamily: 'Inter', background: '#f0f4f9', width: '100%', paddingTop: '20px', justifyContent: 'center'}}>
...
const CreateArtObjectBlock = React.memo(({controller}: {controller: CanvasController}) => {
	...
	return (
		<div style={{display: 'flex', gap: '10px', padding: '7px', borderRadius: '5px'}}>
			<button className = "button" style={{fontFamily: 'Inter', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', borderRadius: '20px', padding: '10px'}} onClick={() => onCreateArt('rectangle')}>{renderIcon('rectangle')}{' Rectangle'}</button>
			<button className = "button" style={{fontFamily: 'Inter', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', borderRadius: '20px', padding: '10px'}} onClick={() => onCreateArt('triangle')}>{renderIcon('triangle')}{' Triangle'}</button>
			<button className = "button" style={{fontFamily: 'Inter', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', borderRadius: '20px', padding: '10px'}} onClick={() => onCreateArt('ellipse')}>{renderIcon('ellipse')}{' Ellipse'}</button>
		</div>
	)
})
...
const renderIcon = (type: ArtObjectType): JSX.Element => {
	switch (type) {
		case 'rectangle':
			return <div style={{width: 16, height: 16, marginRight: 5}}><RectangleShape /></div>
		case 'triangle':
			return <div style={{width: 16, height: 16, marginRight: 5}}><TriangleShape /></div>
		case 'ellipse':
			return <div style={{width: 16, height: 16, marginRight: 5}}><EllipseShape /></div>
		default:
			return <></>
	}
}
...
const ImageImportBlock = React.memo(({controller}: {controller: CanvasController}) => {
	...
	return (
		<div style={{display: 'flex', padding: '7px', borderRadius: '5px'}}>
			<input
				...
				style={{display: 'none'}}
			/>
			<button className = "button"style={{fontFamily: 'Inter', border: 'none', borderRadius: '20px', padding: '10px'}}>
...
const JsonBlock = React.memo(({controller, model, onLoad}: {controller: CanvasController, model: ICanvasReadModel, onLoad: () => void}) => (
	<div style={{display: 'flex', gap: '10px', padding: '7px', borderRadius: '5px'}}>
		<button
			className = "button"
			style={{fontFamily: 'Inter', border: 'none', borderRadius: '20px', padding: '10px'}}
			onClick={() => {
...
const ShapeControlBlock = React.memo(({selectedShapeId, handleDeleteShape}: {
	...
	return (
		<div style={{display: 'flex', gap: '10px', padding: '7px', borderRadius: '5px'}}>
			<button className = "button"style={{fontFamily: 'Inter', border: 'none', borderRadius: '20px', padding: '10px'}} onClick={() => handleDeleteShape(selectedShapeId)}>
				{'Delete shape'}
			</button>
		</div>
	)
